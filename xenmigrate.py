...
def vmdktoraw(vmdkfile, rawfile, gz):
    ...
    if debug:
        ...
        print('gzip              :', gz)
    if (not gz and not os.path.exists(rawfile)) or (
            (gz and not os.path.exists(rawfile + '.gz')) and (gz and not os.path.exists(rawfile))):
        try:
...
if __name__ == '__main__':
    ...
    parser.add_option('-c', '--convert', action='store', type='string', dest='convert', metavar='DIR',
                      help='convert DIR or vmdk to importable rawfile')
    ...
    parser.add_option('-i', '--import', action='store', type='string', dest='doimport', metavar='FILE',
                      help='import from FILE to [type=xen:importVolGroup]|\n[type=xenserver:importdiskuuid]')
    parser.add_option('-l', '--lvdev', action='store_true', dest='lvdev', help='display vm logical volume devices',
                      default=False)
    parser.add_option('-t', '--type', action='store', type='string', dest='type', metavar='TYPE',
                      help='import to [xen]|[xenserver]', default='xen')
    parser.add_option('-x', '--export', action='store', type='string', dest='export', metavar='FILE',
                      help='export from Xen Server or from Logical Volume dev to FILE')
    parser.add_option('-z', '--gzip', action='store_true', dest='gz', help='use compression for import, export, or convert (SLOW!)',
                      default=False)
    ...
    if opts.export and opts.doimport:
        ...
        if opts.type == 'xen':
            ...
            print('to logical volume :', lvdest)
            print('on volume group   :', lvpartid)
            print('with gz           :', opts.gz)
            importvm(lvdest, opts.doimport, lvpartid, lvsize, opts.gz)
        elif opts.type == 'xenserver':
            vmdiskuuid = importname.split('_')[-2]
            lvsize = importname.split('_')[-1]
            print('import raw file   :', opts.doimport)
            print('to disk uuid      :', vmdiskuuid)
            print('on vm uuid        :', vmuuid)
            print('with gz           :', opts.gz)
            importxenserverdisk(opts.doimport, vmdiskuuid, vmuuid, opts.gz)
    elif opts.convert:
        if not os.path.exists(opts.convert):
            print('ERROR: convert directory ' + opts.convert + ' does not exist')
            sys.exit(1)
        rawfile = args[0]
        if opts.gz:
            rawfile = rawfile + '.gz'
        print('convert directory :', opts.convert)
        print('to raw file       :', rawfile)
        print('with gz           :', opts.gz)
        reftoraw(opts.convert, rawfile, opts.gz)
    else:
        print('ERROR: no valid commands given')
        parser.print_help()
        sys.exit(1)
